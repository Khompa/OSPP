...
JOB_ÃŒNCREMENT_HELLO:	.asciiz "Job started running increment\n"
...
# Buffer info for gets
buffer: 		.asciiz " zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz\""
#buffer_2: 		.space 100 # -------------------------------------------------------------
#buffer_3: 		.space 100 # -------------------------------------------------------------
...
job_increment_infinite_loop_short:
	addi $s0, $s0, 1
	j job_increment_infinite_loop_short
...
job_gets_infinite_loop:
	...
	li $a1, 7		# set buffer size
	la $a0, buffer		# get buffer address
	li $v0, 8		# System call code (8) gets
	teqi $zero, 0    
...
# Keep track of buffer offset
buffer_offset: 		.word 0 #----------------------------------------------------------------
...
TODO_1: # Save $a0, $a1, $s0 to the context. 
        sw $a0, 8($k1)         # $a0 -------------------------------------- (enligt line 413 så är k1 address of running task state struct)
        sw $a1, 12($k1)         # $a1 --------------------------------------
        sw $s0, 16($k1)         # $s0 --------------------------------------
...
__system_call_gets:
	...
	# To resume at the instruction following the teqi instruction, 
	# add 4 to EPC and store the result in $t1. 
	
	# TIP: Use the addi (Add Immediate) instruction. 
	addi $t1, $t0, 4 # ----------------------------------------------------------
	
	# The value of EPC + 4 must now be saved in the context of the caller. 
	...
	addi $s0, $zero, -1
	sw $s0, buffer_offset # återställ buffer offset till 0      ---4 ----------------------------------------------------------------------------
...
__gets_system_call_pending: # detta förstör getc -------------------------------------------------------------------------------------------------------------
...
TODO_8:	# Before resuming the waiting job, put the ASCII value of the pressed
	...
	#Increment buffer offset when we have recieved a character
	
	lw $k1, buffer_offset 
	addi $k1, $k1, 1
	sw $k1, buffer_offset

	#Branch if recieved character is enter
	li $k1, 0xa	# 0xa is ascii value for enter
	beq $v0, $k1, string_full_enter
	
	la $k1, buffer
	lw $k0, buffer_offset
	add $k1, $k1, $k0	
	sb $v0, ($k1) #job context is restored so we have access to $a0 
	

	# branch if offset == size 
 	addi $s0, $a1, 0
	addi $s0, $s0, -2
	lw $k0, buffer_offset($zero)  
	beq $s0, $k0, string_full_size
	
string_not_full:
	lw $k0, __running
	# $k0 holds id of job running for input, restore the context of this job. 
 ...
 string_full_enter: 
 	...
 	# SKRIV NULL PÅ SLUTET
 	la $k1, buffer
	lw $k0, buffer_offset
	add $k1, $k1, $k0
	sb $zero, ($k1) #job context is restored so we have access to $a0 
	j after
 ...
 string_full_size: 
	...
	lw $k0, buffer_offset
	add $k1, $k1, $k0
	sb $zero, 1($k1) #job context is restored so we have access to $a0 
